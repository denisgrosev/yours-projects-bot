...
COST_TEXT = (
    ...
    "– Мы сотрудничаем с ЮKassa, это гарантирует безопасность и простоту платежей"
)
...
REF_TEXT = (
    ...
    "– Деньги можно вывести на карту или перевести в баланс для заказов.\n"
...
REF_MENU = InlineKeyboardMarkup([
    [InlineKeyboardButton("📄 Пример работы", callback_data="welcome_start")],
...
def get_example_keyboard(page):
    ...
    if row:
        keyboard.append(row)
    keyboard.append([InlineKeyboardButton("🟢 Начать", callback_data="example_skip")])
...
async def welcome_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "welcome_cost":
        ...
        try:
            ...
            log_event("fallback_edit_error", error=str(e), chat_id=chat_id)
            await context.bot.send_message(
                chat_id=chat_id,
                text=WELCOME_TEXT,
                reply_markup=WELCOME_MENU
